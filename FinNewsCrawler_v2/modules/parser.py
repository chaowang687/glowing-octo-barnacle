...
class NewsParser(DataParser):
    ...
    def parse_eastmoney_news(data: Dict, keyword: str = "") -> List[Dict]:
        ...
        try:
            ...
            # 方式1: 直接从 cmsArticle 获取
            if 'cmsArticle' in data:
                articles = data['cmsArticle']
            # 方式2: 从 result.cmsArticle 获取
            elif 'result' in data and 'cmsArticle' in data['result']:
                articles = data['result']['cmsArticle']
            # 方式3: 从 data 直接获取（可能是列表）
            elif isinstance(data, list):
                articles = data
            
            # 调试信息
            logger.info(f"尝试解析新闻数据，找到 {len(articles)} 条新闻")
            ...
            for item in articles:
                try:
                    ...
                    # 尝试不同的字段名获取标题和日期
                    title = NewsParser.clean_html(item.get('title', item.get('Title', '')))
                    pub_date = NewsParser._normalize_date(item.get('date', item.get('Date', '')))
                    
                    news = {
                        'stock_code': NewsParser._extract_stock_code(item, keyword),
                        'stock_name': NewsParser._extract_stock_name(item),
                        'title': title,
                        'content': NewsParser.clean_html(item.get('content', item.get('Content', ''))),
                        'pub_date': pub_date,
                        'source': item.get('source', item.get('Source', '东方财富')),
